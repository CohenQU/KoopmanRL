...
# load data
#******************************************************************************
Xtrain, Xtest, Xtrain_clean, Xtest_clean, indices_train, indices_test, state_dim, action_dim = data_from_name(args.dataset, noise=args.noise, theta=args.theta, train_size=args.train_size, test_size=args.test_size)
...
for i in np.arange(args.steps,-1, -1):
    ...
    records = []
    if i == 0:
        if len(indices_train) > 1:
            for j in range(1, len(indices_train)):
                if tmp == None:
                    tmp = Xtrain[indices_train[0]+start:indices_train[j]]
                    records.append((indices_train[0]+start, indices_train[j]))
                else:
                    tmp = torch.cat((tmp, Xtrain[indices_train[j-1]+start:indices_train[j]]), 0)
                    records.append((indices_train[j-1]+start,indices_train[j]))
            tmp = torch.cat((tmp, Xtrain[indices_train[j]+start:]), 0)
            records.append((indices_train[j]+start,len(Xtrain)))
            trainDat.append(tmp.float())
        else:
            trainDat.append(Xtrain[indices_train[0]+start:].float())
    else:
        if len(indices_train) > 1:
            for j in range(1, len(indices_train)):
                if tmp == None:
                    tmp = Xtrain[indices_train[0]+start:indices_train[j]-i]
                    records.append((indices_train[0]+start, indices_train[j]-i))
                else:
                    tmp = torch.cat((tmp, Xtrain[indices_train[j-1]+start:indices_train[j]-i]), 0)
                    records.append((indices_train[j-1]+start,indices_train[j]-i))
            tmp = torch.cat((tmp, Xtrain[indices_train[j]+start:-i]), 0)
            records.append((indices_train[j]+start,len(Xtrain)-i))
            trainDat.append(tmp.float())
        else:
            trainDat.append(Xtrain[indices_train[0]+start:-i].float())
    start += 1
...
# Model
#==============================================================================
model = koopmanAE(state_dim, action_dim, args.bottleneck, args.steps, args.steps_back, args.alpha, args.init_scale)
print('koopmanAE')
...
# Start training
#==============================================================================
model, optimizer, epoch_hist = train(model, train_loader, state_dim, action_dim,
...
if len(indices_test) > 1:
    for j in range(1, len(indices_test)):
        if Xinput == None:
            Xinput = Xtest[indices_test[0]:indices_test[j]-1]
            Xtarget = Xtest_clean[indices_test[0]+1:indices_test[j]]
        else:
            Xinput = torch.cat((Xinput, Xtest[indices_test[j-1]:indices_test[j]-1]), 0)
            Xtarget = torch.cat((Xtarget, Xtest_clean[indices_test[j-1]+1:indices_test[j]]), 0)

    Xinput = torch.cat((Xinput, Xtest[indices_test[j]:-1]), 0)
    Xtarget = torch.cat((Xtarget, Xtest_clean[indices_test[j]:-1]), 0)
else:
    Xinput = Xtest[indices_test[0]:-1]
    Xtarget = Xtest_clean[indices_test[0]+1:]

# print("Xinput shape: ", Xinput.shape)
# print("Xtarget shape: ", Xtarget.shape)
# exit()
...
for i in range(len(indices_test)):
    # print("i = {}".format(i))
    start = indices_test[i]
    if i == len(indices_test) - 1:
        end = args.test_size
    else:
        end = indices_test[i+1]
    Xinput = Xtest[start:end-1]
    Xtarget = Xtest_clean[start+1:end]
    error_temp = []

    init = Xinput[0][:,:state_dim,:]
    actions = Xinput[:,:,state_dim:,:]

    if i == 0:
        init0 = init
    z = model.encoder(init) # embedd data in latent space
    length = end - start - 1
    length = min(length, args.pred_steps)
    #print("start: ", start, "end: ", end, "length: ", length)
    for j in range(length):
        
        a = actions[j].reshape(-1, 1, action_dim)
        #print("a.shape: ", a.shape)
        #print("before cat, z.shape: ", z.shape)
        z = torch.cat((z, a), dim=2)
        #print("after cat, z.shape: ", z.shape)
        z = model.dynamics(z)
        #print("after dynamics, z.shape: ", z.shape)
    ...
    #         x_pred = model.decoder(z) # map back to high-dimensional space
        target_temp = Xtarget[j][:,:state_dim,:].data.cpu().numpy().reshape(state_dim,1)
        error_temp.append(np.linalg.norm(x_pred.data.cpu().numpy().reshape(state_dim,1) - target_temp) / np.linalg.norm(target_temp))
        
        if i == 0:
            snapshots_pred.append(x_pred.data.cpu().numpy().reshape(state_dim,1))
            snapshots_truth.append(target_temp)
    for j in range(length, args.pred_steps):
        error_temp.append(error_temp[-1])
...
import scipy
save_preds = {'pred' : np.asarray(snapshots_pred), 'truth': np.asarray(snapshots_truth), 'init': np.asarray(init0.float().to(device).data.cpu().numpy().reshape(state_dim,1))} 
