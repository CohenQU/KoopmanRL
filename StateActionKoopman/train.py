...
def train(model, train_loader, state_dim, action_dim, lr, weight_decay, 
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, data_list in enumerate(train_loader):
            model.train()
            initial_state = data_list[0][:,:,:state_dim,:].to(device)
            actions = [data_list[0][:,:,state_dim:,:].to(device)]
            for k in range(1, steps):
                actions = actions + [data_list[k][:,:,state_dim:,:].to(device)]
            out, out_back = model(initial_state, actions, mode='forward')
        
            for k in range(steps):
                if k == 0:
                    loss_fwd = criterion(out[k], data_list[k+1][:,:,:state_dim,:].to(device))
                else:
                    loss_fwd += criterion(out[k], data_list[k+1][:,:,:state_dim,:].to(device))

            
            loss_identity = criterion(out[-1], data_list[0][:,:,:state_dim,:].to(device)) * steps
